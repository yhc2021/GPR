...
struct PointXYZII {
    ...
    PointXYZII(float _x, float _y, float _z,float _intensity, float _index) : x(_x), y(_y), z(_z), intensity(_intensity),index(_index) {}
};
...
{   
    ...
    vector<PointXYZII> regionwise_ground;
    
    RevertCandidate(int _c_idx, int _s_idx, double _flatness, double _line_var, Eigen::VectorXf _pc_mean, vector<PointXYZII> _ground)
...
struct Params 
{
    ...
    Params() {
        ...
        RNR_intensity_thr = 0.0;    // Noise points intensity threshold. The reflected points have relatively small intensity than others.
...
public:
    typedef std::vector<vector<PointXYZII>> Ring;
    ...
    Eigen::MatrixXf getGround() { return toEigenCloud(cloud_ground_); }
    Eigen::MatrixXf getNonground() { return toEigenCloud(cloud_nonground_); }
    
    Eigen::MatrixX3f getCenters() { return toEigenCloud3(centers_); }
    Eigen::MatrixX3f getNormals() { return toEigenCloud3(normals_); }
...
private:
    ...
    vector<PointXYZII> ground_pc_, non_ground_pc_;
    vector<PointXYZII> regionwise_ground_, regionwise_nonground_;

    vector<PointXYZII> cloud_ground_, cloud_nonground_;

    vector<PointXYZ> centers_, normals_;

    Eigen::MatrixXf toEigenCloud(vector<PointXYZII> cloud);
    Eigen::MatrixX3f toEigenCloud3(vector<PointXYZ> cloud);

    void addCloud(vector<PointXYZII> &cloud, vector<PointXYZII> &add);
    ...
    double calc_point_to_plane_d(PointXYZII p, Eigen::VectorXf normal, double d);
    ...
    void estimate_plane(const vector<PointXYZII> &ground);
