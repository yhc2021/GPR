...
bool point_z_cmp(PointXYZII a, PointXYZII b) { return a.z < b.z; }

Eigen::MatrixX3f PatchWorkpp::toEigenCloud3(vector<PointXYZ> cloud)
{
...
Eigen::MatrixXf PatchWorkpp::toEigenCloud(vector<PointXYZII> cloud)
{
    Eigen::MatrixXf dst(cloud.size(), 5);
    int j=0;
    for (auto &p: cloud) {
        dst.row(j++) << p.x, p.y, p.z, p.intensity, p.index;
    }
    return dst;
}

void PatchWorkpp::addCloud(vector<PointXYZII> &cloud, vector<PointXYZII> &add)
{
...
void PatchWorkpp::estimate_plane(const vector<PointXYZII> &ground) {

    if (ground.empty()) return;

    Eigen::MatrixXf eigen_ground(ground.size(), 5);
    int j=0;
    for (auto &p: ground) {
        eigen_ground.row(j++) << p.x, p.y, p.z, p.intensity, p.index;
    }
    Eigen::MatrixX3f centered = eigen_ground.leftCols(3).rowwise() - eigen_ground.leftCols(3).colwise().mean();
    Eigen::MatrixX3f cov = (centered.adjoint() * centered) / double(eigen_ground.rows() - 1);
    
    pc_mean_.resize(3);
    pc_mean_ << eigen_ground.colwise().mean()(0), eigen_ground.colwise().mean()(1), eigen_ground.colwise().mean()(2); //eigen_ground.colwise().mean()(3),eigen_ground.colwise().mean()(4);
...
void PatchWorkpp::extract_initial_seeds(
        const int zone_idx, const vector<PointXYZII> &p_sorted,
        vector<PointXYZII> &init_seeds, double th_seed) {
    ...
    for (int zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx) {
        ...
        for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
                ...
                // if (params_.verbose){
                //     cout << "pc_mean_" << i << ": "<<pc_mean_(i)<<"     normal_" << i << ": "<<normal_(i) << endl;
                // }
                }
                // if (params_.verbose) cout << "ring_idx:" << ring_idx << "  sector_idx:" << sector_idx  << "  heading:" << heading << endl;
                ...
                */
                ...
                bool is_heading_outside = heading < params_.heading_thr;
...
void PatchWorkpp::reflected_noise_removal(Eigen::MatrixXf &cloud_in)
{
    ...
    for (int i=0; i<cloud_in.rows(); i++)
    {
        ...
        {
            cloud_nonground_.push_back(PointXYZII(cloud_in.row(i)(0), cloud_in.row(i)(1), cloud_in.row(i)(2), cloud_in.row(i)(3), cloud_in.row(i)(4)));
                                         ...
                                         int concentric_idx)
{
    ...
    {        
        ...
        if (params_.verbose) 
        {
            cout  << "prob_line:" << prob_line << "  prob_flatness:" << prob_flatness << "  revert:" << revert << endl;
        }
        if (params_.TGR_all_accept) revert = true;
...
void PatchWorkpp::extract_piecewiseground(
        const int zone_idx, const vector<PointXYZII> &src,
        vector<PointXYZII> &dst,
        vector<PointXYZII> &non_ground_dst) {
    ...
    vector<PointXYZII> src_wo_verticals;
    ...
    if (params_.enable_RVPF)
    {
        for (int i = 0; i < params_.num_iter; i++)
        {
            ...
            if (zone_idx == 0 && normal_(2) < params_.uprightness_thr)      // zone_idx == 0 是最近的同心圆区域
            // if (normal_(2) < params_.uprightness_thr)
            {
                vector<PointXYZII> src_tmp;
...
double PatchWorkpp::calc_point_to_plane_d(PointXYZII p, Eigen::VectorXf normal, double d)
{
...
void PatchWorkpp::pc2czm(const Eigen::MatrixXf &src, std::vector<Zone> &czm) {
    ...
    for (int i=0; i<src.rows(); i++) {
        ...
        float x = src.row(i)(0), y = src.row(i)(1), z = src.row(i)(2), intensity = src.row(i)(3), index = src.row(i)(4);
        ...
        if ((r <= max_range) && (r > min_range)) {
            ...
            if (r < min_range_1) { // In First rings
                ...
                czm[0][ring_idx][sector_idx].emplace_back(PointXYZII(x, y, z, intensity, index));
                ...
                czm[1][ring_idx][sector_idx].emplace_back(PointXYZII(x, y, z, intensity, index));
                ...
                czm[2][ring_idx][sector_idx].emplace_back(PointXYZII(x, y, z, intensity, index));
            } else { // Far!
                ...
                czm[3][ring_idx][sector_idx].emplace_back(PointXYZII(x, y, z, intensity, index));
            }

        } else {
            cloud_nonground_.push_back(PointXYZII(x, y, z, intensity, index));
        }
    }
