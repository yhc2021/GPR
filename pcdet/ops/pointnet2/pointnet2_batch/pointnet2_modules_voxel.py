        ...
        if ctr_xyz is None:
            ...
            for i in range(len(self.sample_type_list)):
                ...
                if xyz_tmp.shape[1] <= npoint: # No downsampling
                    ...
                    # tb = time.time()
                    # print('FPS time: ', tb-ta)

                elif 'voxel' in sample_type or 'Voxel' in sample_type:
                    sample_idx = pointnet2_utils.furthest_point_sample(xyz_tmp.contiguous(), npoint, point_cloud_range = [-75.2, -75.2, -2, 75.2, 75.2, 4], voxel_size=[0.4, 0.4, 0.6])
