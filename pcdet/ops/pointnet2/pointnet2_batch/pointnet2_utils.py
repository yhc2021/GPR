...
class VoxelPointSampling(Function):
    @staticmethod
    def forward(ctx, xyz: torch.Tensor, npoint: int, point_cloud_range = [-75.2, -75.2, -2, 75.2, 75.2, 4], voxel_size=[0.4, 0.4, 0.6]) -> torch.Tensor:
        """
        Uses iterative farthest point sampling to select a set of npoint features that have the largest
        minimum distance
        :param ctx:
...
Voxel_point_sample = voxel_point_sample = VoxelPointSampling.apply
