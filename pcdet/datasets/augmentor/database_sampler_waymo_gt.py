    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                print("use_shared_memory !!!!!!!!!!!")
            else:
                # file_path = self.root_path / info['path']
                file_path = info['path']

                if 'kitti' in file_path:
                    ...
                    obj_points = np.fromfile(str(file_path), dtype=np.float32).reshape(-1, 5)[:,0:self.sampler_cfg.NUM_POINT_FEATURES]
                    assert obj_points.shape[0] == info['num_points_in_gt']
                        # obj_points = np.fromfile(str(file_path), dtype=np.float64).reshape(-1, 5)[:,0:self.sampler_cfg.NUM_POINT_FEATURES]
