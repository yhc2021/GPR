...
class IASSD_Backbone(nn.Module):
    ...
    def painting_points(self,batch_dict):
        ...
        for batch_id in range(batch_size):
            ...
            cat_features = torch.zeros(
                (cur_points.shape[0], 1),
                dtype=cur_points.dtype,
                device=cur_points.device)
            cur_gt_boxes = gt_boxes[batch_id]
            gt_boxes_mask = cur_gt_boxes[:,-1] == 2
            cur_gt_boxes = cur_gt_boxes[gt_boxes_mask][:,:-1]
            box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(
                cur_points[:, 1:4].unsqueeze(dim=0).float().cuda(),
                cur_gt_boxes[:, 0:7].unsqueeze(dim=0).float().cuda()
            ).long().squeeze(dim=0)
            # point_masks = torch.from_numpy(roiaware_pool3d_utils.points_in_boxes_cpu(cur_points[:, 1:4], cur_gt_boxes)).cuda()
            cat_features[box_idxs_of_pts >= 0] = 1
            cur_points = torch.hstack((cur_points,cat_features))
            cur_points_list.append(cur_points)
        points = torch.cat(cur_points_list,dim=0)
    ...
    def forward(self, batch_dict):
        ...
        Returns:
            batch_dict:
                ...
                point_features: (N, C)
        """
        # batch_dict = self.painting_points(batch_dict)
        batch_size = batch_dict['batch_size']
